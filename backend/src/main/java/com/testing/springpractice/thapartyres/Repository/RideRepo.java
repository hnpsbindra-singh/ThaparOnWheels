...
public interface RideRepo extends MongoRepository<Rides, String> {
    ...
    List<Rides> findByStatus(RideStatus status);
    List<Rides> findByUserIdAndStatus(String userId, RideStatus status);
    List<Rides> findByDriverIdAndStatus(String driverId, RideStatus status);
    List<Rides> findAllByOrderByCreatedAtDesc();
}
